...
from maskrcnn_benchmark.engine.trainer import do_train, do_train_set
...
def train(cfg, local_rank, distributed, zero_shot, skip_optimizer_resume=False, save_config_path = None):
    ...
    if cfg.HJ.UPDATE_CENTERNESS_NETWORK:
        for p in model.rpn.head.centerness.parameters():
            p.requires_grad = True
    
    if cfg.HJ.USE_PREBUILT_CENTERNESS:
        model.prebuilt_centerness_level1.requires_grad = True
        model.prebuilt_centerness_level2.requires_grad = True
        model.prebuilt_centerness_level3.requires_grad = True
        model.prebuilt_centerness_level4.requires_grad = True
        model.prebuilt_centerness_level5.requires_grad = True
