...
class GLIPDemo(object):
    ...
    def run_on_web_image(self, 
            ...
            custom_entity = None,
            alpha = 0.0,
            return_centerness=False,
            vldyhead_only_return_centerness=False,
            not_post_process=False):
        if vldyhead_only_return_centerness and (not return_centerness):
            raise Exception(f"if return_centerness == False then vldyhead_only_return_centerness also should be False")
        
        if return_centerness:
            predictions, visual_features, centerness, fused_visual_features = self.compute_prediction(original_image, original_caption, custom_entity, return_centerness=True,vldyhead_only_return_centerness=vldyhead_only_return_centerness)
            top_predictions = self._post_process(predictions, thresh)
        else:
            predictions, visual_features, fused_visual_features = self.compute_prediction(original_image, original_caption, custom_entity)
        ...
        if return_centerness:
            return result, top_predictions, visual_features, centerness, fused_visual_features
        return result, top_predictions, visual_features, fused_visual_features
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity = None, return_centerness = False, vldyhead_only_return_centerness = False):
        """
            return_centerness, vldayhead_only_return_centerness : HJ add
        """
        # image
        ...
        with torch.no_grad():
            if self.cfg.MODEL.RPN.RETURN_FUSED_FEATURES:
                if return_centerness:
                    predictions, visual_features, centerness, fused_visual_features = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token, return_centerness=True)
                else:
                    predictions, visual_features, fused_visual_features = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token)

                predictions = [o.to(self.cpu_device) for o in predictions]
                visual_features = [o.to(self.cpu_device)[0] for o in visual_features]
                fused_visual_features = [o.to(self.cpu_device)[0] for o in fused_visual_features]
            else:
                if return_centerness:
                    predictions, centerness = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token)
                else:
        ...
        # print("visual features shape", [visual_features[k].shape for k in range(len(visual_features))])
        # input("ckpt!")
        ...
        if return_centerness:
            return prediction, visual_features, centerness, fused_visual_features
        return prediction, visual_features, fused_visual_features
