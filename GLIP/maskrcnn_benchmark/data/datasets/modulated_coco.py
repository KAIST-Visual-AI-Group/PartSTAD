...
class CocoGrounding(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        print("img shape modulated_coco.py line 232", img.shape)
...
# class CocoGrounding_Set(torchvision.datasets.CocoDetection):
#     """
#         HJ modified for make one object as one data (not an image as one data)
...
#                  **kwargs
#                  ):
#         super(CocoGrounding_Set, self).__init__(img_folder, ann_file)
#         self.ids = sorted(self.ids)
...
#         # input("ckpt modulated_coco.py line 347")
#         self.dirs = ann['dirs']
#         self.data_per_set = data_per_set #TODO to be changed
#         self.extra_dict_list = []
#         for dir_dict in self.dirs:
#             data_dir = dir_dict['data_dir']
#             fname = data_dir.split("/")[-1]
#             preprocess_dir = dir_dict['preprocess_dir']
#             glip_dir = dir_dict['glip_dir']
#             info_pre_dir = dir_dict['info_pre_dir']

#             pc_file = f"{data_dir}/pc.ply"
#             xyz, rgb = normalize_pc_o3d(pc_file, None, None)
#             label = np.load(f"{data_dir}/label.npy", allow_pickle=True)
#             label_dict = label.item()
#             gt_sem_seg = label_dict['semantic_seg']

#             superpoint = np.load(f"{preprocess_dir}/superpoint.npy", allow_pickle=True)
#             bbox_info = np.load(f"{info_pre_dir}/bbox_info_compressed.npz", allow_pickle=True)
#             sp_info = np.load(f"{preprocess_dir}/sp_info.npz", allow_pickle=True)
...
#             with open(f"{glip_dir}/glip_pred/pred.json", "r") as f:
#                 glip_pred = json.load(f)
#             extra_dict = {"fname":fname,
#                           "xyz":xyz,
...
#                          }
#             self.extra_dict_list.append(extra_dict)

#             del gt_sem_seg, superpoint, bbox_info, sp_info
...
#         target_list = []
#         idx_list = []
#         extra_dict = self.extra_dict_list[idx]
#         for i in range(10):
#             img, target, idx_ = self.get_single_item(idx*self.data_per_set + i)
#             img_list.append(img)
#             target_list.append(target)
#             idx_list.append(idx_)

#         # img_tensors = torch.stack(img_tensors, 0) # 10, 3, 800, 800
#         # new_img_list = to_image_list(img_tensors)
#         # new_target_list = cat_boxlist(target_list) # BoxList 
#         # new_idx = idxs # list

#         return img_list, target_list, idx_list, extra_dict


    
#     def __len__(self):
#         if len(self.ids)%self.data_per_set != 0:
#             raise Exception(f"data_per_set should divide len(self.ids). modulated_coco.py line 362, len(self.ids):{len(self.ids)}, self.data_per_set:{self.data_per_set}")
#         return (len(self.ids)//self.data_per_set)

#     def get_single_item(self, idx):
#         img, tgt = super(CocoGrounding_Set, self).__getitem__(idx)
#         image_id = self.ids[idx]
#         tgt = [obj for obj in tgt if obj["iscrowd"] == 0]
#         boxes = [obj["bbox"] for obj in tgt]
#         boxes = torch.as_tensor(boxes).reshape(-1, 4)  # guard against no boxes
