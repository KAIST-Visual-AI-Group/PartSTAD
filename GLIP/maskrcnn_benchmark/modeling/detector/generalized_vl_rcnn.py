...
class GeneralizedVLRCNN(nn.Module):
    ...
    def forward(self, 
        ...
        return_fused_feature=False, # Since original output is backbone feature, we change is to fused feature
        return_only_iou_dict=False,
        ...
        if 'vl' in self.cfg.MODEL.SWINT.VERSION:
            ...
            #PartSLIP here (both train, test)
            visual_features = self.backbone(images.tensors) #backbone: swint.py
        ...
        if self.force_boxes:
            ...
            #PartSLIP train, inference here

            #RPN architecture : VLDYHEAD
            # proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets, language_dict_features, positive_map,captions, swint_feature_c4) #proposals : Boxes, original code

            if self.cfg.HJ.RETURN_CENTERNESS: # HJ add
                proposals, proposal_losses, fused_visual_features, centerness = self.rpn(images, visual_features, targets, language_dict_features, positive_map,captions, swint_feature_c4, vldyhead_only_return_centerness=vldyhead_only_return_centerness)

            else:
                proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets, language_dict_features, positive_map,captions, swint_feature_c4) #proposals : Boxes

            # VLDYHEAD forward 함수에 다 있음


            # print("proposals", proposals)
            # print("fused_visual_features", fused_visual_features)
            # input("ckpt! generalized_vl_rcnn.py line 335")
        ...
        if self.roi_heads:
            ...
            if self.cfg.MODEL.ROI_MASK_HEAD.PREDICTOR.startswith("VL"):
                if self.training:
                    print("generalized_vl_rcnn line 345")
            ...
            if self.cfg.MODEL.RPN.RETURN_FUSED_FEATURES:
                print("generalized_vl_rcnn line 354")
        ...
        #NOTE: fused_visual_features는 output에 없었는데 내가 추가한거임
        if self.cfg.MODEL.RPN.RETURN_FUSED_FEATURES:
            if return_centerness:
                return result, visual_features, centerness, fused_visual_features
            return result, visual_features, fused_visual_features

        if return_centerness:
            return result, centerness
