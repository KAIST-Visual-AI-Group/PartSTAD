...
class VLFuse(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # for k in range(len(visual_features)):
        #     print(visual_features[k].shape)
        # input("ckpt! vldyhead.py line 451")
...
class VLDyHead(torch.nn.Module):
    ...
    def forward(self, x, language_dict_features=None, embedding=None, swint_feature_c4=None):
        ...
        for l, feature in enumerate(x):
            """
                X = 100, 50, 25, 13, 7 order
                feature shape : 1, 256, X, X
                logit shape : 1, 80, X, X
                bbox reg shape : 1, 4, X, X
                centerness shape : 1, 1, X, X
                dyhead_tower["visual"] : final visual feature output of dyhead layers
            """
            # print("l : ",l)
            # print("feature shape : ", feature.shape)
            # input("ckpt vldyhead.py line 802")
            logits.append(self.cls_logits(dyhead_tower["visual"][l]))
            # print(self.cls_logits(dyhead_tower["visual"][l]))
            # print(self.cls_logits(dyhead_tower["visual"][l]).shape)
            # print("logits len,",len(logits))
            # input("ckpt vldyhead.py line 805")

            bbox_pred = self.scales[l](self.bbox_pred(dyhead_tower["visual"][l]))
            # bbox pred shape : 1,4,X,X (X = 100, 50, 25, 13, 7 order)
            bbox_reg.append(bbox_pred)
            # print("bbox_pred len,",len(bbox_reg))
            # input("ckpt vldyhead.py line 810")

            centerness.append(self.centerness(dyhead_tower["visual"][l]))
            # print("centerness len,",len(centerness))
            # print(self.centerness(dyhead_tower["visual"][l]).shape)
            # input("ckpt vldyhead.py line 814")
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS: #True!
                ...
                # add bias (language)
                dot_product_proj_queries = self.dot_product_projection_image(x) # Identity!
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None,
        ...
        anchors = self.anchor_generator(images, features) #modeling/rpn/anchor_generator.py AnchorGenerator


        # print(fused_visual_features)
        # print(len(fused_visual_features))
        # for fidx in range(len(fused_visual_features)):
            # print(fused_visual_features[fidx].shape)
        # print("fused visual features above (vldyhead.py line 928)")
        # input("ckpt!")

        if vldyhead_only_return_centerness:
            return None, None, None, centerness
        ...
        if self.training:
            ...
            return self._forward_test(box_regression, centerness, anchors,
                                      ...
                                      positive_map,
                                      fused_visual_features=fused_visual_features,
                                      vldyhead_only_return_centerness=vldyhead_only_return_centerness
                                      )
    ...
    def _forward_train(self, box_cls, box_regression, centerness, targets, anchors,
        ...
        if self.cfg.HJ.USE_ONLY_RIOU_LOSS:
            losses = {}
        ...
        if self.cfg.MODEL.RPN_ONLY:
            #PartSLIP Training here
            # return None, losses, None # original

            if self.cfg.HJ.RETURN_CENTERNESS: #HJ add
            ...
            boxes = self.box_selector_train(box_regression, centerness, anchors,
                                        ...
                                        ) # ATSSPostProcessor in modeling/rpn/inference.py
            train_boxes = []
            ...
            if self.cfg.HJ.RETURN_CENTERNESS:
                return train_boxes, losses, fused_visual_features, centerness
            else:
    ...
    def _forward_test(self, box_regression, centerness, anchors,
        ...
        if self.cfg.HJ.RETURN_CENTERNESS:
            return boxes, {}, fused_visual_features,centerness
