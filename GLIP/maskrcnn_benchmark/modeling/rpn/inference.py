...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
        ...
        if dot_product_logits is not None:
            # PartSLIP here
            dot_product_logits_before_sigmoid = dot_product_logits.clone()
            ...
            if self.mdetr_style_aggregate_class_num != -1:
                ...
                # print("scores")
                # print(scores[0,0,:10])
                #Hyunjin add to save logits before sigmoid
                logits = convert_grounding_to_od_logits(logits=dot_product_logits_before_sigmoid, box_cls=box_cls,
                                                        positive_map=positive_map,
                                                        score_agg=self.score_agg, init_val=-1000)
                # print("logits")
                # print(logits[0,0,:10])
                # print("logits.sigmoid()")
                # print(logits.sigmoid()[0,0,:10])
                # input("ckpt")
            box_cls = scores
            box_cls_before_sigmoid = logits #Hyunjin add

        # print("box_regression shape", box_regression.shape) # 1, 4, H, W
        # W = box_regression.shape[3]
        # print("box regression [0,:,10,15]", box_regression[0,:,10,15])
        H,W = box_regression.shape[2:]
        # print("box regression [0,:,7270//136,7270%136]", box_regression[0,:,7270//136,7270%136])
        # box_regression_clone = box_regression.clone()
        box_regression = permute_and_flatten(box_regression, N, A, 4, H, W)
        # print("box_regression shape", box_regression.shape) # 1, HW, 4
        # print("box regression [0,:,10,15]", box_regression[0,W*10+15,:])
        # input("ckpt!")
        # N, A, H, W = 1, 1, X, X (X = 100, 50, 25, 13, 7)
        box_regression = box_regression.reshape(N, -1, 4) # 1, M^2, 4

        candidate_inds = box_cls > self.pre_nms_thresh # 1, M^2, 80
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)

        centerness = permute_and_flatten(centerness, N, A, 1, H, W) # 1, M^2, 1
        centerness_before_sigmoid = centerness.clone().reshape(N,-1) # 1, M^2
        centerness = centerness.reshape(N, -1).sigmoid() # 1, M^2
        # print("centerness type", centerness.dtype, centerness[0])
        # print("centerness_before_sigmoid type", centerness_before_sigmoid.dtype, centerness_before_sigmoid[0])
        # input("ckpt")
        # multiply the classification scores with centerness scores

        # print("box_cls[0], centerness[0]", box_cls[0], centerness[0])
        # print("box_cls[0], centerness[0]", box_cls_before_sigmoid.sigmoid()[0], centerness_before_sigmoid.sigmoid()[0])
        box_cls = box_cls * centerness[:, :, None] # 1, M^2, 80
        

        # box_cls_before_sigmoid = box_cls_before_sigmoid #Hyunjin add : extra data 1  
        # centerness_before_sigmoid = centerness_before_sigmoid #Hyunjin add : extra data 2

        results = []

        for per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors, per_box_cls_before_sigmoid, per_box_centerness_before_sigmoid \
                in zip(box_cls, box_regression, pre_nms_top_n, candidate_inds, anchors, box_cls_before_sigmoid, centerness_before_sigmoid):
            per_box_cls = per_box_cls[per_candidate_inds]
            per_box_cls_before_sigmoid = per_box_cls_before_sigmoid[per_candidate_inds]
            ...
            # print("box idx:",per_box_loc[:1]) #7270, 7271, 7272, 7406, 
            # print("per_box_regression[7270]", per_box_regression[7270])
            # input("ckpt")
            per_class = per_candidate_nonzeros[:, 1] + 1 #class label of each box

            # print(per_class)
            # print("len per_box_loc", len(per_box_loc))
            ...
            per_box_centerness_before_sigmoid = per_box_centerness_before_sigmoid[per_box_loc]
            # print("per_box_cls_before_sigmoid", per_box_cls_before_sigmoid.shape)
            # print("per_box_centerness_before_sigmoid", per_box_centerness_before_sigmoid.shape)
            # if len(per_box_cls) > 1:
            #     print("score[0]", per_box_cls[0])
            #     print("sanity", per_box_cls_before_sigmoid.sigmoid()[0]*per_box_centerness_before_sigmoid.sigmoid()[0])
            #     print("score[0]", per_box_cls[1])
            #     print("sanity", per_box_cls_before_sigmoid.sigmoid()[1]*per_box_centerness_before_sigmoid.sigmoid()[1])
            #     input("ckpt")

            boxlist = BoxList(detections, per_anchors.size, mode="xyxy")

            ###TODO Hyunjin Add###
            boxlist.add_field("index", per_box_loc)
            per_box_loc_HW = torch.stack([per_box_loc//W, per_box_loc%W],-1)
            boxlist.add_field("pos",per_box_loc_HW)
            boxlist.add_field("level", torch.ones(per_box_loc.shape[0])*level)
            boxlist.add_field("logit_before_sigmoid", per_box_cls_before_sigmoid) #M^2, 80 
            boxlist.add_field("centerness_before_sigmoid", per_box_centerness_before_sigmoid) #M^2, 1 
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        # box_cls : logit # 80 x M x M
        # box_regression : box_reg # 4 x M x M
        # token_logits : None
        # dot_product_logits : 80 x M x M
...
def convert_grounding_to_od_logits(logits, box_cls, positive_map, score_agg=None, init_val=0):
    scores = torch.ones(logits.shape[0], logits.shape[1], box_cls.shape[2]).to(logits.device)*init_val
    ...
    if positive_map is not None:
        ...
        if score_agg == "MEAN":
            ...
            for label_j in positive_map:
                # print(f"positive_map[{label_j}]", positive_map[label_j])
                # input(":ckpt")
