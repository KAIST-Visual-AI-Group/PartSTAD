...
def load_non_normalized_pc(pc_file, save_dir, io, device, save_normalized_pc=False):
...
def normalize_pc(pc_file, save_dir, io, device, save_normalized_pc=False):
    ...
    # print(f"norm max : {torch.norm(xyz, dim=1, p=2).max().item()}")
...
def normalize_pc_o3d(pc_file, save_dir, device, save_normalized_pc=False):
    # pc = io.load_pointcloud(pc_file, device = device)
...
def get_iou(bb1, bb2):
    ...
    float
        in [0, 1]
    """
    assert bb1['x2'] - bb1['x1'] >= 0
    assert bb1['y2'] - bb1['y1'] >= 0
    assert bb2['x2'] - bb2['x1'] >= 0
    assert bb2['y2'] - bb2['y1'] >= 0
